...
void CV::circle(float x, float y, float radius, int div)
{
   ...
   float inc = PI_2 / div;
   glBegin(GL_LINE_LOOP);
   for (int lado = 1; lado <= div; lado++) // GL_LINE_LOOP desenha um Polygon2d fechado. Liga automaticamente o primeiro e ultimio vertices.
   {
      x1 = (cos(ang) * radius);
      y1 = (sin(ang) * radius);
...
void CV::circleFill(float x, float y, float radius, int div)
{
   ...
   float inc = PI_2 / div;
   glBegin(GL_POLYGON);
   for (int lado = 1; lado <= div; lado++) // GL_POLYGON desenha um Polygon2d CONVEXO preenchido.
   {
      x1 = (cos(ang) * radius);
      y1 = (sin(ang) * radius);
      glVertex2d(x1 + x, y1 + y);
